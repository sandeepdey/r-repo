...
select * from transactional.pbl_authorization where med_id = 574215;


select * from dwh.dim_ndc_hierarchy where label_name_long ilike '%metformin 500 mg%';



select * from fdb.rmiid1_med where lower(med_medid_desc) = lower('METFORMIN 500 MG ORAL TAB');

select * from fdb.rmidfid1_routed_dose_form_med where lower(med_routed_df_med_id_desc) = lower('METFORMIN 500 MG ORAL TAB');

select * from fdb.rndc14_ndc_mstr where lower(ln) ilike '%METFORMIN 500 MG ORAL TAB%';

select * from medispan.mf2tcgpi 
inner join medispan.mf2name on mf2tcgpi.tcgpi_id = mf2name.generic_product_identifier
where tcgpi_name ilike  '%Citalopram Hydrobromide Tab 10 MG%';
